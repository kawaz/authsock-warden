...
|---|---|---|---|
| `op://[VAULT[/ITEM]]` | warden がローカル署名 | op CLI で遅延取得 → メモリ | TouchID |
| `agent:PATH` or socket パス | upstream に転送 | upstream が保持 | upstream 任せ |
| `file:PATH` or 通常ファイルパス | warden がローカル署名 | ファイルから読み込み → メモリ | パスフレーズ |

`op://` 記法は 1Password 公式の secret reference 形式と一致。`op://` で全鍵、`op://vault` で vault 限定、`op://vault/item` で特定鍵。
...
```
source "work" (members: op://emerada, agent:/path/to/1password.sock, file:~/.ssh/id_work)
...
2. `op item list --categories SSH_KEY` → fingerprint-to-itemid マップ（TouchID 1回）
3. キャッシュヒット → 即座に公開鍵解決
4. 1Password agent socket → REQUEST_IDENTITIES → fingerprint 照合
5. 残り → `op item get` 並列実行
6. キャッシュ更新保存

### 2回目以降の REQUEST_IDENTITIES
1. 1Password agent socket に問い合わせ（即座、TouchID なし）
2. 新しい鍵を検出 → キャッシュとマッチングして追加
3. `op item list` は再実行しない

### 署名時
- 秘密鍵はメモリにキャッシュ（初回のみ `op item get --reveal`）
- 2回目以降の署名は即座
...
- CLI: clap (derive)
- 設定: serde + toml
- セキュリティ: zeroize, libc (mlock/ptrace)
