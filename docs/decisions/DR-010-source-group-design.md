...
| 記法 | 意味 |
|---|---|
| `op:` | 1Password（op CLI で鍵管理、warden がローカル署名） |
| `agent:PATH` | SSH agent ソケットに転送（明示指定） |
| `file:PATH` | 秘密鍵ファイルから読み込み（明示指定） |
| `PATH`（プレフィックスなし） | パスの種類で自動判定: socket → agent、通常ファイル → file |
