...
DR-018 で構想する「soft TTL → TouchID で延長 / hard TTL → zeroize」のキャッシュ戦略は **まだ実装されていない**。v0.1.28 時点で `OpManagedKey::cached_pem` は次の状態:

- TTL なし。プロセスが生きている間 PEM がメモリに残り続ける（`zeroize` on drop はある）
- `tokio::sync::Mutex` で per-key serialize されているので、cold cache での TouchID 二重発火は防げる
- soft TTL / hard TTL の切替・再認証ロジックは未実装
- `warden kv` サブコマンドは未実装。CLI から KV を操作する手段がない

つまり今の signer / warden_proxy は「DR-018 の adapter 層側」だけが先に整っている状態。KV コア（TTL/再認証/Unix socket API）はこのDRで設計を固めて実装する。
...
- 既存 `cached_pem` を新 KV コアに置き換える移行戦略（一気に置き換える / 段階的に併存させる）
